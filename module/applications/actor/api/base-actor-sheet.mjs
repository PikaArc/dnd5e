...
export default class BaseActorSheet extends PrimarySheetMixin(
  ...
  async _prepareItems(context) {
    ...
    context.items = Array.from(this.actor.items)
      ...
      // TODO: Does this need sorting here, or can inventory handle that?
      .forEach(async item => {
        // Prepare item context
        const ctx = context.itemContext[item.id] ??= {};
        this._prepareItem(item, ctx);
        if ( item.type === "spell" ) this._prepareItemSpell(item, ctx);
        else if ( "quantity" in item.system ) this._prepareItemPhysical(item, ctx);
        else this._prepareItemFeature(item, ctx);
  ...
  _prepareSkillsTools(context, property) {
    ...
    //   let src = source.system[property]?.[key]?.ability;
    //   if ( src ) return src;
